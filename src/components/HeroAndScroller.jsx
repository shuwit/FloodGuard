...
// ─────────────────────────────────────────────
//  TYPING ANIMATION COMPONENT & PHRASES
// ─────────────────────────────────────────────
...
const TypewriterText = ({ phrases }) => {
  ...
  React.useEffect(() => {
    ...
    const handleType = () => {
      ...
      // Typing speed: normal is 100ms, deleting is faster (50ms)
...
const Screen0 = () => (
  <div className="w-full h-full bg-white flex flex-col items-center justify-center">
    {/* Removed the stray 'j' and fixed the background to bg-white */}
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center p-5 gap-2 bg-[#f8fafc]">
      ...
      <div className="w-full flex items-center justify-center gap-6 mt-1">
        ...
        <div className="relative w-12 h-[150px] bg-gray-200 rounded-full overflow-hidden shadow-inner border-[3px] border-white">
          ...
          {/* Threshold Markers inside the bar (50% and 70%) */}
          ...
          {/* The Animated "Water" Level */}
          <motion.div
            className="absolute bottom-0 left-0 w-full z-0 rounded-full"
            initial={{ height: '10%', backgroundColor: '#16a34a' }}
            animate={isActive ? {
              // It goes up to 65% (Warning), spikes to 92% (High), drops to 45% (Low), and back!
              height:          ['10%', '65%', '92%', '45%', '15%'],
              backgroundColor: ['#16a34a', '#d97706', '#dc2626', '#16a34a', '#16a34a']
            } : { height: '10%' }}
            transition={{
              duration: 8, // Takes 8 seconds to complete one full cycle
              repeat: Infinity,
              ease: "easeInOut",
              times: [0, 0.3, 0.6, 0.8, 1] // Ensures the colors sync perfectly with the height
            }}
          />
        </div>
...
const Screen2 = ({ isActive }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center p-5 gap-3 bg-[#0f172a] pt-12" style={{ paddingTop: '140px' }}>
      ...
      {/* 👇 FIX: Deleted the CSS and replaced it with a Framer Motion Bell! 👇 */}
      <motion.div 
        className="relative text-[40px] mb-2 select-none"
        style={{ transformOrigin: 'top center' }} // Makes it swing from the top like a real bell
        animate={isActive ? { 
        ...
        transition={{ 
          duration: 1, 
          repeat: Infinity, 
          repeatDelay: 1.5 // Rings, pauses for 1.5 seconds, then rings again!
        }}
      >
        🔔
        {/* Added animate-pulse to the red dot for extra urgency */}
...
const HeroAndScroller = () => {
  ...
  // EXACT MOVEMENT ARROWS:
  // Center -> Right (Purple) -> Left (Green) -> Right (Orange)
  ...
  // EXACT STARTING POSITION (Red Down Arrow):
  // Starts pushed down 22vh so it sits below the hero text perfectly.
  ...
  // Phone rotates its "cheeks" to face the text
  ...
  // Blue Circle behind the phone ONLY in the hero section
  const circleOpacity = useTransform(scrollYProgress, [0, 0.15], [1, 0]);
  const circleScale = useTransform(scrollYProgress, [0, 0.15], [1, 0.5]);

  // "Here's how it works" title fades in as user scrolls down
  const scrollerTitleOpacity = useTransform(scrollYProgress, [0.15, 0.25], [0, 1]);

  // Smooth springs to make the movement fluid
  const smoothX       = useSpring(rawX,       { stiffness: 50, damping: 15, mass: 0.5 });
  const smoothY       = useSpring(rawY,       { stiffness: 50, damping: 15, mass: 0.5 });
  const smoothRotateY = useSpring(rawRotateY, { stiffness: 50, damping: 15, mass: 0.5 });
  ...
  return (
    // The background color of the scroller section
    <section ref={containerRef} className="relative bg-[#e0f2fe]" style={{ height: '500vh', marginBottom:'30px' }}>

      {/* ── BACKGROUND VIDEO CONTAINER (Scrolls up naturally!) ── */}
      ...
      <div className="absolute top-20 left-0 w-full h-[100vh] z-10 flex flex-col items-center justify-start pt-[12vh] px-6">
        <h1 className="text-4xl md:text-5xl lg:text-[80px]  font-extrabold text-white text-center leading-[1.2] tracking-tight mb-4 drop-shadow-md w-full">
          ...
          {/* Top Line */}
          <div className="mb-2">
            <span className="underline decoration-white underline-offset-8">FloodGuard</span> can help you
          </div>
          
          {/* Bottom Line: The Invisible Box Fix */}
          <div className="flex justify-center items-center" style={{marginLeft:'100px'}}>
            <span className="whitespace-nowrap">with&nbsp;</span>
            {/* 👇 FIX: Added whitespace-nowrap and increased widths to hold huge words! 👇 */}
            <span className="inline-block text-left w-[320px] md:w-[550px] lg:w-[750px] whitespace-nowrap">
              <TypewriterText phrases={helpPhrases} /><span className="animate-pulse">|</span>
            </span>
          </div>

        </h1>

        <p className="text-white/60 text-lg md:text-xl font-medium mb-8 text-center drop-shadow mt-4 tracking-wide" style={{maxWidth:'700px', marginTop:'10px'}}>
          Anticipate hazards to prepare safely and mitigate impacts.
        </p>
      </div>
      ...
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden pointer-events-none"
        ...
        <div className="absolute inset-0 w-full max-w-[1920px] mx-auto flex items-center justify-center pointer-events-none z-20"
          ...
          {sections.map((s, i) => {
            ...
            return (
              <div
                key={sIdx}
                className={`absolute w-[85%] md:w-[380px] xl:w-[460px] transition-all duration-700 pointer-events-auto`}
                style={{
        ...
        <motion.div 
          style={{ 
            ...
            scale: circleScale,
            // 👇 Added the exact Canva radial gradient here!
            backgroundImage: 'radial-gradient(circle at 0% 0%, #99bfe6, #04478f)',
            marginTop:'450px',
          }}
          // 👇 Removed the solid bg-[#1d4ed8] from here
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{paddingTop:'30px'}}>
          ...
          >
            {/* 👇 NEW: 3D Extrusion Layers (Creates the thick dark metal side) 👇 */}
            ...
            {/* Side buttons (Pushed back to sit on the side edge) */}
            ...
            <div className="absolute inset-0 bg-[#09090b] rounded-[40px] border-[6px] border-[#3f3f46] p-[6px]" style={{ transform: 'translateZ(0px)' }}>
              <div className="relative w-full h-full bg-white rounded-[36px] overflow-hidden">
                ...
                {/* LCD Screens */}
                <div className="absolute inset-0 transition-opacity duration-700" style={{ opacity: activeSection === 0 ? 1 : 0 }}><Screen0 /></div>
                <div className="absolute inset-0 transition-opacity duration-500 delay-150" style={{ opacity: activeSection === 1 ? 1 : 0, pointerEvents: activeSection === 1 ? 'auto' : 'none' }}><Screen1 isActive={activeSection === 1} /></div>
                <div className="absolute inset-0 transition-opacity duration-500 delay-150" style={{ opacity: activeSection === 2 ? 1 : 0, pointerEvents: activeSection === 2 ? 'auto' : 'none' }}><Screen2 isActive={activeSection === 2} /></div>
                <div className="absolute inset-0 transition-opacity duration-500 delay-150" style={{ opacity: activeSection === 3 ? 1 : 0, pointerEvents: activeSection === 3 ? 'auto' : 'none' }}><Screen3 /></div>
